...
<!-- HOWTO-build-and-release.md is generated from HOWTO-build-and-release.Rmd. Please edit that file -->

# Staging area — ARPC team only

This folder is the **local workbench** for preparing, reviewing, and
releasing a publication’s data bundle. The built bundles under
`data-raw/published-data/` are git-ignored; the scripts, catalog, and
release bodies in `data-raw/` are tracked. Bundles themselves are
distributed only as ZIP assets on GitHub Releases.
...
## Layout
    ...
    └── scripts/
        ...
        └── release-body/release-body-<TYPE>.Rmd  # release-page body per type
...
1.  **Initialize the release pages once** (and whenever a body changes):
    ...
    `scripts/release-body/release-body-<TYPE>.Rmd` and creates/refreshes
    the `report`, `brief`, `working-paper`, and `white-paper` releases.
